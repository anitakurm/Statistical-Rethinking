...
```
  The model fit correct, but why does the posterior look so weird?   
    The posterior is the answer: what is the value of knowing length of the right leg, when you already know the left?
```{r}
post <- extract.samples(m5.8)
plot(bl~br, post, col=col.alpha(rangi2, 0.1), pch=16)
```
    Posterior distribution of the association of each leg with height,from model 5.8
    Since both variables contain almost identical information, the posetrior is a narrow ridge of negatively correlated values
    When bl is large, then br must be small
    two predictors with identical information -> there will be practically infinite possible combinations of parameters  br and bl that produce the same predictions
    
    yi ~ Normal(mui, sigma)
    mui= apha+ beta1 *xi + beta2*xi        left leg - xi, right leg is the same,so it's also xi
    
    so computer does this, it factors xi out of each term:
    yi ~ Normal(mui, sigma)
    mui= apha+ (beta1+ beta2)*xi 
    
    now beta 1 and beta 2 influnce mu, only as a sum, they cannot be pulled apart
    -> infinite combinations of beta1 and beta2 that makes their sum close to actual association of x with y
    
    compute the posterior of the sum of beta 1 and beta 2
```{r}
#find the sum
sum_blbr <- post$bl+post$br

#plot posterior distribution of the sum
dens(sum_blbr, col=rangi2, lwd=2, xlab="sum of bl and br")

```
    The posterior distribution of the sum of the two parameters is centered on the proper association of either leg with height
    The density plot of posterior distribution (with the mean of a bit of above 2, and small sd) is basically the same, as the posterior of a regression that has only one of the leg variables
```{r}
#fit a model
m5.9 <- map(
  alist(
...
#plot posterior distribution of the sum
dens(post$bl, col=rangi2, lwd=2, xlab="bl")
```
    basically the same estimates, as the posterior of the sum of left and right
    
    The lesson: When two predictor variables are verys trongly correlated, including both in a model may lead to confusion
    The posterior distribution is not wrong, it just can't answer the question with these data
    
    5.3.2.Multicollinear milk
    we might miss the fact that predictors are highly correlated in the real data -> we may mistakenly read the posterior distribution to say that neither predictor is important 
    
```{r}
#primate milk data again 
data(milk)

d <- milk
```
    
    We try to model the total energy content of the milk, kcal.per.g, by using perc.fat and perc.lactose <- natural case of multicollinearity 
    
    Two bivariate regressions first
```{r}
#fit a model
...
m5.10 <- map(
  alist(
    ...
    mu <- a + bf*perc.fat ,
    a ~ dnorm( 0.6 , 10 ) ,
    bf ~ dnorm( 0 , 1 ) , #beta measures association of a fat perc with energy content 
    sigma ~ dunif( 0 , 10 )
    ) ,
  data=d )
...
m5.11 <- map(
  alist(
    ...
    a ~ dnorm( 0.6 , 10 ) ,
    bl ~ dnorm( 0 , 1 ) , #beta measures association of a fat perc with energy content 
    sigma ~ dunif( 0 , 10 )
    ) ,
  data=d )
...
```
    The posterior mean for bf (the association of percent fat with milk energy) is 0.01, with 89% interval [0.008,0.012]
    The posterior mean for bl (the association of percent lactose with energy) is -0.01, with 89% interval [-0.012,-0.009]
    
      These posterior means are mirror images of one another (bf mean is as positive, as bl mean is negative)
      Both are narrow posterior distributions liýing on either side of zero
    
  Slopes (parameters for beta values): small in absolute value 
      Are these associations really good for modeling energy content of milk?
      Yes! The predictor variables are percentages, potentially large values
      The absolute magnitude of regression slopes is not always meaningful 
        <- because the influence on prediction depends upon the product of the parameter and the data
        you gotta compute+plot predictions or standardize all predictors -> probably still plot implied predictions 
        
  -> Strong association of each predictor with the outcome -> both variables are reliable predictors of total energy in milk, across species
    the more fat - the more energy
    the more lactose - the fewer kcal in milk 
    
    Now together:
```{r}
#kcal.per.g regressed on perc.fat and perc.lactose
m5.12 <- map(
  alist(
    kcal.per.g ~ dnorm( mu , sigma ) ,
...
```
    The posterior means of both bf anf bl are closer to zero. (especially bf)
    StdDevs are twice as large as in bivariate regressions
    
    <- The variables perc.fat and perc.lactose contain much of the same information.They are substitutes for one another 
    You include both in the same model -> the posterior distribution ends up describing a long ridge of combinations of bf and bl that are equally possible 
    Fat and lactose form essentially a single axis of variation (see pairs plot)
```{r}
#pairs plot to see how perc.fat and per.lactose for a single axis of variation
pairs( ~ kcal.per.g + perc.fat + perc.lactose, data=d, col=rangi2)
```
    Along diagonal: variables are labeled
    In each scatterplot off the diagonal: the vertical axis is the variable labeled on the same row, the horizontal axis is the variable labeled in the same column
    
    First row. 
     Left: kcal.per.g. (vertical) against perc.fat (horizontal) - positive correlation
     Right: kcal.per.g (vertical) against per.lactose (horizontal) - negative correlation
    
    Middle row:
      Right: scatter of percent fat (vertical) against percent lactose (horizontal)
            the points line up almost entirely along a straight line ->
            -> these variables are so strongly negatively correlated, they are nearly redundant (we can omit one of them without loss of information)
            
    
    A pairs plot of the total energy, perecent fat, and percent lactose variables from the primate milk data.     Percent fat and percent lactose are strongly negatively correlated with one another, providing mostly the same information.
    Either helps in predicting kcal.per.g, but neither helps much once you already know the other
    
    Compute correlation:
```{r}
#compute correlation between two predictor variables
cor(d$perc.fat, d$perc.lactose)
```
    A pretty strong correlation 
    It's hard to say how strong the correlation has to get before we should worry about it.. <- Pretty high
    Matters!!: The correlation that remains after accounting for any other predictors
    
    Let's address the cor question directly: a little simulation experiment
    we have only kcal.per.g and perc.fat 
```{r}
#we have only kcal.per.g and perc.fat
#construct a random predictor variable - x, that is correlated  with per.fat at predetermined level 
#code on p 150

#write a function that generates correlated predictors
sim.coll <- function( r=0.9) {
  d$x <- rnorm(nrow(d), mean= r*d$perc.fat,
               sd=sqrt((1-r^2)*var(d$perc.fat)))
  m<- lm(kcal.per.g ~ perc.fat + x, data=d) #fit a model
  sqrt(diag(vcov(m) ))[2] #return std dev of parameter (sd of the posterior dstrb for the slope btwn fat and kcal)
}

#repeatedly call the previous function, with diffrent degrees of correlation as an input, collect the results
rep.sim.coll <- function(r=0.9, n=100) {
  stddev <- replicate(n, sim.coll(r))
  mean(stddev)
}
r.seq <- seq(from=0, to=0.99, by=0.01) #define all the correlation values
stddev <- sapply(r.seq, function(z) rep.sim.coll(r=z,n=100)) #generate 100 regressions for each value in r.seq, return the avg std dev from them
plot(stddev~ r.seq, type= "l", col=rangi2, lwd=2, xlab="correlation")
```
    The effect of correlated predictor variables on the narrowness of the posterior dstrb
    y-axis: std dev of the posterior dstrb of the slope
    x-axis: the correlation btwn the predictor of interest and another predictor added to the model
    The correlation increases -> The std dev inflates (it approaches infintiy as correlation approaches 1)
      When variables are not correlated , the stddev of the posterior is small -> it's a narrow range of most plausible parameter values
    
    Multicolinearity - the best we can do is be aware of it
      anticipate by checking predictors against each other in a pairs plot
      
    You can't know from a table of correlations nor a matrix of scatterplots whether multicolinearity will prevent you from including sets of variables in the same model
    Diagnose the problem by looking for a big inflation of stddev when both variables are included in the same model
    
    Dealing with collinear variables:
      some fields - data reduction procedures, e.g. principle components, factor analysis -> then use the components/factors as predictor variables
                so hard to interpret tho!!!
                component might not tell immediately how any real measurement is associated with the outcome
      A defensive approach - show that you get the same inferences and predictions by using any one of the highly correlated variables
      Multicollinearity - a Non-identifiability family problem 
                a parameter is non-identifiable -> the structure of the data and model do not make it possible to estimate the parameter's value 
                sometimes it's a mistake in coding a model
                many important types of models present weakly or non-indentifiable parameters even if the code is alright
                a very wide posterior dstrb - the available data does not containy much info about a parameter of interest 
                it's usually helpful to check covariation among parameters, expecially when intervals are extremely wide, to avoid misinterpretation of the model output
                

    5.3.3. Post-treatment bias
      Omitted varianle bias - mistaken inferences that arise from omitting predictor variables
      "post-treatment" - from experimental designs, and observational studies
      
      You want to know the difference in growth under different anti-ffungal soil treatments (fungus tends to reduce the growth)
      goal: make a causal inference about the treatment -> you shouldn't include the presence of fungus, bacause it is a post-treatment effect
...
precis(m5.13)
```
    simulation variance bla bla bla - every sample is special
    bt - treatment had very little effect on growth
    bt - fungus! and bh -h0 (initial height) have important effects
    
    but we know that treatment matters - we simulated it this way, what happened?
      fungus is a consequence of treatment
        Once we already know whether or not a plant developed fungus, does soil treatment matter? No.
        Treatment has its effects on growththrough reducing fungus.
        Design of the experiment: we want to know the impact of TREATMENT on growth -> we should omit the post-treatment variable fungus
```{r}
#fit a model without fungus variable
m5.14 <- map(
  alist(
    h1 ~ dnorm( mu , sigma ) ,
...
precis(m5.14)
```
   Now bt - impact of treatment on growth - is strong and positive (as it should be)
   makes sense: controling for pre-treatment differences (like initial height) that might mask the causal influence of treatment
   doesn't: including post-treatment variables that can actually mask the treatment itself
   
   it is easier with experimental design, than observational studies
   
  5.4. Categorical variables
  taxonomic group - categorical variable (e.g. sex, developmental status, geogrphic region) - factors
  How are these variables included in a model?
  Understand that to be able to easily interpret estimates for categorical variables 
    5.4.1 Binary categories
    just two categories: sex
```{r}
data("Howell1")
d <- Howell1
str(d)
```
    male variable - new predictor - an example of a dummy variable
    Dummy variables are devices for encoding categories into quantitative models
      the effect of a dummy var - turn a parameter on for those cases in the category
    1 -male, 0 - female or 1 -female, 0 - male <- YOU NEED TO REMEMBER IN ORDER TO INTERPRET
